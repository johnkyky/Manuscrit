...
#let perf_domain_0 = csv("../../../data/trahrhe_perf_domain_0.csv", row-type: dictionary)
...
#let plot = {
 ...
 lq.diagram(
        ...
        ..for (i, trahrhe_type) in ("exact", "gist_exact", "dicho", "gist_dicho").enumerate() {
          (lq.plot(
            perf_domain_0.filter(row => row.at(trahrhe_type) != "").map(row => int(row.at("N"))),
            perf_domain_0.filter(row => row.at(trahrhe_type) != "").map(row => float(row.at(trahrhe_type))),
