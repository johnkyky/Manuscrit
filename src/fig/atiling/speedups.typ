...
#let plot = {
  ...
  let xlabels = xl_dataset.map(row => {
    if "best_pesto" in row and row.at("best_pesto") == "hybrid" {
      return row.at("benchmark") + "*"
    }
    return row.at("benchmark")
  })
  ...
  let static_speedups = xl_dataset.map(row => float(row.at("time_pluto_static")) / float(row.at("time_pesto")))
  let dynamic_speedups = xl_dataset.map(row => float(row.at("time_pluto_dynamic")) / float(row.at("time_pesto")))
