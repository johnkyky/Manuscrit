...
#cetz.canvas(
  length: 0.6cm,
  {
    ...
    let split1(pos, ..args) = group({
      line(
        pos,
        (rel: (tilewidth - 1, 0)),
        (rel: ((tileheight - 1) * 2, tileheight - 1)),
        (rel: (-(tilewidth + 4 * (tileheight - 1) - 1), 0)),
        close: true,
        fill: cgreen,
        ..args,
      )
    })
    let split2(pos, ..args) = group({
      line(
        pos,
        (rel: ((4 * (tileheight - 1)) - 1, 0)),
        (rel: (-((4 * (tileheight - 1)) - 1) / 2, ((4 * (tileheight - 1)) - 1) / 4)),
        // (rel: ((tileheight - 1) * 2 - 1, tileheight - 1.5)),
        // (rel: ((tileheight - 1) * 2 - 1, -tileheight + 1.5)),
        close: true,
        fill: cpurple,
        ..args,
    ...
    for tx in range(0, 2) {
      split1((4 + tx * (tilewidth + 4 * (tileheight - 1)), 0))
    }
    split2((6, 0))

    for tx in range(0, 2) {
      split2((1 + tx * (tilewidth + 4 * (tileheight - 1)), 3))
    }
    split1((9, 3))
    ...
    for t in range(0, 3) {
      line((-0.5, 3 * t - 0.5), (N - 0.5, 3 * t - 0.5), stroke: (paint: black, dash: "dashed"))
    }
