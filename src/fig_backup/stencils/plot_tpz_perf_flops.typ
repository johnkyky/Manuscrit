...
#let plot = {
  ...
  let datasets = (
    tpz_data.filter(row => row.at("tags") == "pluto:static").map(row => float(row.at("Mflops"))),
    tpz_data.filter(row => row.at("tags") == "pluto:dynamic").map(row => float(row.at("Mflops"))),
    tpz_data.filter(row => row.at("tags") == "tpz:pesto").map(row => float(row.at("Mflops"))),
  )
      ...
      (
        ..xs
          ...
          .map(((x, y)) => {
            ...
            lq.place(x + (0.3 * i - 0.3), y, pad(0.2em, text(size: 8pt, [#calc.round(y, digits: 0)])), align: align)
          }),
      )
    },
  )
