...
#let canvas(N: 6, label-size: 12pt, label-offset: -0.2cm, fontSize: 8pt, radius: 0.1cm, ..args) = cetz.canvas(..args, {
  import cetz.draw: *
  ...
  for i in range(1, N + 1) {
    for j in range(1, N + 1) {
      if i <= N + 2 - j {
        circle((i, j), radius: radius, fill: blue, stroke: none)
      }
    }
  }
})
#canvas(length: 0.8cm, radius: 0.2cm, fontSize: 12pt)