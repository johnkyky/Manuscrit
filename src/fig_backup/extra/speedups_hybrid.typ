...
#let plot = {
  ...
  let static_speedups = xl_dataset.map(row => float(row.at("time_pluto_static")) / float(row.at("time_pesto_hybrid")))
  let dynamic_speedups = xl_dataset.map(row => float(row.at("time_pluto_dynamic")) / float(row.at("time_pesto_hybrid")))
  ...
  let xticks = xlabels.map(el => rotate(-45deg, reflow: true, el)).enumerate()
  let xs = range(0, xticks.len())

  let barLabels = ("Pesto (hybrid) vs Pluto (static)", "Pesto (hybrid) vs Pluto (dynamic)")

  let formatter(ticks, ..args) = {
    let result = ticks
  ...
  lq.diagram(
    ...
    xlim: (-1, xticks.len()),
    ylim: (0, 2),
    xaxis: (ticks: xticks, subticks: none, format-ticks: formatter),
