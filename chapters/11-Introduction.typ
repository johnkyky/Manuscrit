...
The compiler serves as the critical bridge between the high-level abstractions written by the developer and the low-level instructions executed by the machine. In addition, conversion to an executable binary file, the compiler is responsible for applying specific optimizations. Ideally, a developer could rely entirely on the compiler's internal heuristics and optimization passes to achieve maximum performance. However, to maximize the utilization of hardware ressources for compute-intensive HPC codes, this is rarely sufficient.

An alternative approach is to perform manual code transformations and optimizations directly at the source level to assist the compiler. While a well-executed manual optimization often yields superior performance, it demands significant expertise, increases development time, and severely degrades code maintainability. It effectively requires the developer to be both a domain scientist and a low-level code optimization expert. To overcome this bottleneck, performance portability frameworks offer a alternative to obtain performance without development expertise cost. They allow developers to simply express their computational kernels, while the framework handles the complex mapping and deployment of these kernels to maximize performance across the chosen hardware targets.
